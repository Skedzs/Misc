...
FluentIcons["lucide-user-round"] = FluentIcons["lucide-user"]


local function GetIcon(name)
    if not name or name == "" then return "" end
    if string.find(name, "rbxassetid://") then return name end
    if string.find(name, "http") then return name end
    
    if FluentIcons then
        local iconId = FluentIcons["lucide-" .. name] or FluentIcons[name]
        if iconId then return iconId end
    end
    return "rbxassetid://10723407389" -- fallback to default settings icon if not found
end
 do
...
local function OpenClose()
  ...
  if TargetParent:FindFirstChild("Skedz_SpeedHubX_Logo") then
      TargetParent:FindFirstChild("Skedz_SpeedHubX_Logo"):Destroy()
  end
  local ScreenGui = Custom:Create("ScreenGui", {
    Name = "Skedz_SpeedHubX_Logo",
  ...
  local Close_ImageButton = Custom:Create("ImageButton", {
    ...
    Image = "rbxassetid://96768511234361",
    Visible = false,
  }, ScreenGui)
...
function Speed_Library:CreateWindow(Config)
  ...
  Min.Activated:Connect(function()
		...
		local tween = TweenService:Create(MainScale, tweenInfo, {Scale = 0})
		tween:Play()
		tween.Completed:Connect(function()
  ...
  local ConfirmationOverlay = Custom:Create("Frame", {
  ...
  Custom:Create("UICorner", {}, ConfirmationOverlay)
  
  local ConfirmationModal = Custom:Create("Frame", {
  ...
  Custom:Create("UICorner", { CornerRadius = UDim.new(0, 8) }, ConfirmationModal)
  Custom:Create("UIStroke", { Color = Color3.fromRGB(50, 50, 50), Thickness = 1.5 }, ConfirmationModal)
  
  local WarningIcon = Custom:Create("ImageLabel", {
      Name = "WarningIcon",
      BackgroundTransparency = 1,
  ...
  local ConfirmText = Custom:Create("TextLabel", {
      Name = "ConfirmText",
  ...
  local ButtonContainer = Custom:Create("Frame", {
  ...
  local YesBtn = Custom:Create("TextButton", {
      Name = "YesBtn",
      Font = Enum.Font.GothamBold,
      Text = "Yes",
      TextColor3 = Color3.fromRGB(255, 255, 255),
  ...
  Custom:Create("UICorner", { CornerRadius = UDim.new(0, 6) }, YesBtn)
  Custom:Create("UIStroke", { Color = Color3.fromRGB(50, 50, 50), Thickness = 1 }, YesBtn)
  
  local NoBtn = Custom:Create("TextButton", {
      Name = "NoBtn",
      Font = Enum.Font.GothamBold,
      Text = "No",
      TextColor3 = Color3.fromRGB(255, 255, 255),
      ...
      BackgroundColor3 = Color3.fromRGB(25, 25, 25),
      Size = UDim2.new(0.4, 0, 0.6, 0),
      Position = UDim2.new(0.54, 0, 0.2, 0),
      ZIndex = 1002
  }, ButtonContainer)
  Custom:Create("UICorner", { CornerRadius = UDim.new(0, 6) }, NoBtn)
  Custom:Create("UIStroke", { Color = Color3.fromRGB(50, 50, 50), Thickness = 1 }, NoBtn)
  
  local promptState = 0
  
  local function closeUIAndModal()
      local tInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
      TweenService:Create(ConfirmationModal, tInfo, {Size = UDim2.new(0, 0, 0, 0)}):Play()
      local tw = TweenService:Create(ConfirmationOverlay, tInfo, {BackgroundTransparency = 1})
      tw:Play()
      tw.Completed:Wait()
  ...
  Close.Activated:Connect(function()
        ...
        TweenService:Create(ConfirmationOverlay, TweenInfo.new(0.2), {BackgroundTransparency = 0.5}):Play()
        TweenService:Create(ConfirmationModal, tInfo, {Size = UDim2.new(0, 250, 0, 110)}):Play()
  end)
  
  NoBtn.Activated:Connect(function()
      CircleClick(NoBtn, Player:GetMouse().X, Player:GetMouse().Y)
      closeUIAndModal()
  end)
  
  YesBtn.Activated:Connect(function()
      CircleClick(YesBtn, Player:GetMouse().X, Player:GetMouse().Y)
      closeUIAndModal()
  ...
  function Tabs:CreateTab(Config)
    ...
    TabButton.Activated:Connect(function()
      ...
      if FrameChoose and (not CurrentTab or Tab.LayoutOrder ~= CurrentTab.LayoutOrder) then
        ...
        local _FTween = TweenService:Create(FrameChoose, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), { Position = UDim2.new(0, 2, 0, math.floor(TabHeight * 0.3) + (TabHeight + 3) * Tab.LayoutOrder) })
    ...
    function Sections:AddSection(Title, OpenSection)
      ...
      function Item:AddToggle(Config)
        ...
        local function ToggleAnimation(isOn)          
          ...
          local CircleColor = isOn and Color3.fromRGB(18, 18, 18) or Color3.fromRGB(230, 230, 230)
